...
export class SalesRepository {
  ...
  async findDuplicates(input:{tenantId:string;userId:string;membershipId:string;kind:string;firstName?:string;lastName?:string;legalName?:string;registrationNumber?:string;email?:string;phone?:string}):Promise<PartyDuplicateMatch[]>{
  ...
  async archiveImpact(input:{tenantId:string;userId:string;partyId:string;membershipId:string}){return this.database.withContext({tenantId:input.tenantId,userId:input.userId},async client=>(await client.query<{impact:Record<string,number>}>("SELECT app.party_archive_impact($1,$2,$3) impact",[input.tenantId,input.partyId,input.membershipId])).rows[0]?.impact??{});}
  async archiveParty(input:{tenantId:string;userId:string;partyId:string;membershipId:string;reason:string}){return this.database.withContext({tenantId:input.tenantId,userId:input.userId},async client=>(await client.query<{impact:Record<string,number>}>("SELECT app.archive_party($1,$2,$3,$4) impact",[input.tenantId,input.partyId,input.membershipId,input.reason])).rows[0]?.impact??{});}

  private async findDuplicatesWithClient(client:SqlClient,input:{tenantId:string;membershipId:string;kind:string;firstName?:string;lastName?:string;legalName?:string;registrationNumber?:string;email?:string;phone?:string}):Promise<PartyDuplicateMatch[]>{
    ...
    const rows=await client.query<{id:string;display_name:string;party_type:string;email:string|null;phone:string|null;registration_number:string|null;projects:string[];units:string[]}>(
      `SELECT party.id,party.display_name,party.party_type,email.value email,phone.value phone,organization.registration_number,
       ...
       LEFT JOIN LATERAL(SELECT value FROM party_contacts WHERE tenant_id=party.tenant_id AND party_id=party.id AND contact_type='email' AND archived_at IS NULL ORDER BY is_primary DESC,created_at LIMIT 1) email ON true
       LEFT JOIN LATERAL(SELECT value FROM party_contacts WHERE tenant_id=party.tenant_id AND party_id=party.id AND contact_type='phone' AND archived_at IS NULL ORDER BY is_primary DESC,created_at LIMIT 1) phone ON true
       LEFT JOIN LATERAL(SELECT array_agg(DISTINCT project.name ORDER BY project.name) items FROM party_project_links link JOIN projects project ON project.tenant_id=link.tenant_id AND project.id=link.project_id WHERE link.tenant_id=party.tenant_id AND link.party_id=party.id AND link.valid_to IS NULL) projects ON true
       LEFT JOIN LATERAL(SELECT array_agg(DISTINCT unit.code ORDER BY unit.code) items FROM unit_interests interest JOIN units unit ON unit.tenant_id=interest.tenant_id AND unit.id=interest.unit_id WHERE interest.tenant_id=party.tenant_id AND interest.party_id=party.id) units ON true
