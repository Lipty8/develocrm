...
export function buildApp(dependencies: { database: Database; verifier: EntraTokenVerifier; corsAllowedOrigins?:Set<string> }): FastifyInstance {
  ...
  app.post<{Params:{salesCaseId:string};Body:{buyers:Array<{partyId:string;role:"buyer"|"co_buyer";isPrimary:boolean;share?:number|null}>;reason:string;idempotencyKey:string}}>("/v1/sales-cases/:salesCaseId/buyer",async(request,reply)=>{try{const context=await sessionContext(request,dependencies.verifier,repository);if(!context)return reply.code(403).send({error:"Workspace není uživateli přístupný"});return reply.send(await sales.changeBuyer({...context,salesCaseId:request.params.salesCaseId,...request.body}));}catch(error){request.log.warn({err:error,correlationId:request.id,salesCaseId:request.params.salesCaseId},"sales case buyer assignment failed");return reply.code(permissionError(error)?403:409).send({error:permissionError(error)?"Nemáte oprávnění změnit kupujícího.":"Postoupení smlouvy se nepodařilo dokončit.",correlationId:request.id});}});
