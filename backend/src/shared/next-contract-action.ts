...
function resolveNextContractAction(input:SalesProcessInput):NextContractAction{
  ...
  if(signed(ks))return{kind:"none",label:"",reason:"Kupní smlouva je podepsaná. Další etapou je předání."};
...
export function getSalesProcessState(input:SalesProcessInput):SalesProcessProjection{
  ...
  else if(active(ks)){completedThrough=3;activeIndex=4;}
  else if(signed(sbk)){completedThrough=3;activeIndex=4;}
  else if(active(sbk)){completedThrough=2;activeIndex=3;}
  else if(signed(rs)){completedThrough=2;activeIndex=3;}
  else if(active(rs)||["reservation","rs","sbk","ks","handover"].includes(input.salesStage??"")){completedThrough=1;activeIndex=2;}
  ...
  const nextContractAction=resolveNextContractAction(input);
