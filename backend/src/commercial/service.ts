...
export class CommercialService{
  ...
  async nextContractAction(input:Context&{unitId:string}):Promise<NextContractAction&{unitId:string;unitCode:string;salesCaseId:string|null;buyerNames:string[];salesProcess:SalesProcessProjection} >{
    return this.database.withContext({tenantId:input.tenantId,userId:input.userId},async client=>{
      const unit=(await client.query<{id:string;code:string;sales_case_id:string|null;sales_stage:string|null;hold_type:string|null;has_interest:boolean;handover_completed:boolean}>(`SELECT unit.id,unit.code,active_case.id sales_case_id,active_case.current_stage sales_stage,
      ...
      const salesProcess=getSalesProcessState({hasActiveSalesCase:Boolean(unit.sales_case_id),contracts,salesStage:unit.sales_stage,holdType:unit.hold_type,hasInterest:unit.has_interest,handoverCompleted:unit.handover_completed});
