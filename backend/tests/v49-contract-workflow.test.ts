...
async function database(){
  ...
  for(const name of [
    ...
    "0024_unit_payment_and_contract_workflow.sql","0028_contract_workflow_and_buyer_assignment.sql",
  ])await db.exec(await source(`../migrations/${name}`));
...
test("změna kupujícího zachová původní vazbu a aktivita klienta je auditovaná",async()=>{
  ...
  assert.equal((await db.query("SELECT id FROM audit_log WHERE entity_id=$1 AND action='sales_case.buyer_changed'",[salesCase])).rows.length,1);
  await db.close();
});
