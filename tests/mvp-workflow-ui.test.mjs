...
test("kritické MVP akce otevírají skutečné formuláře",async()=>{const source=await readFile(appUrl,"utf8");for(const expected of ["setNewClientOpen(true)","setNewHandoverContext({})","setNewContractOpen(true)","NewClientModal","HandoverScheduleModal","NewContractModal","notify={onNewClient??notify}"])assert.ok(source.includes(expected));});
