...
test("celá historie otevírá záložku jednotky a kontext používá české názvy",()=>{assert.match(crm,/action="Celá historie" onAction=\{\(\) => openTab\("history"\)\}/);assert.match(crm,/Aktivní proces/);assert.match(crm,/DOPORUČENÝ DALŠÍ KROK/);assert.match(crm,/pre_reservation: "Předrezervace"/);assert.match(crm,/salesHistoryLabel\(interest\.result\)/);assert.doesNotMatch(crm,/>\{commercial\?\.stage\?\?/);assert.doesNotMatch(crm,/canonical party/);});
