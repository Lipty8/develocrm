...
test("celá historie otevírá záložku jednotky a duplicitní kontextový CTA blok je odstraněn",()=>{assert.match(crm,/action="Celá historie" onAction=\{\(\) => openTab\("history"\)\}/);assert.doesNotMatch(crm,/DOPORUČENÝ DALŠÍ KROK/);assert.match(crm,/salesHistoryLabel\(interest\.result\)/);assert.doesNotMatch(crm,/>\{commercial\?\.stage\?\?/);assert.doesNotMatch(crm,/canonical party/);});
