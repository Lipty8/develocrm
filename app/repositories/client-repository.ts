...
export interface ClientRepository {
  ...
  findDuplicates(input:Omit<NewPartyInput,"duplicateOverride">):Promise<DuplicateMatch[]>;
  ...
  changeBuyer(input:{salesCaseId:string;buyers:Array<{partyId:string;role:"buyer"|"co_buyer";isPrimary:boolean;share?:number|null}>;reason:string;idempotencyKey:string}):Promise<{id:string}>;
}
export class ApiClientRepository implements ClientRepository {
  ...
  async findDuplicates(input:Omit<NewPartyInput,"duplicateOverride">){const endpoint="/api/clients/duplicates";try{const response=await apiFetch(endpoint,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(input)});if(response.status===503&&responseAllowsBrowserFallback(response))return[];if(!response.ok){const payload=await response.json().catch(()=>({})) as {correlationId?:string;error?:string};const correlationId=response.headers.get("x-correlation-id")||payload.correlationId||"neuvedeno";console.error(JSON.stringify({event:"clients.duplicate_check.failed",endpoint,status:response.status,correlationId}));throw new Error("Kontrolu duplicit se nepodařilo provést. Zkuste to prosím znovu.");}return((await response.json()) as {matches:DuplicateMatch[]}).matches;}catch(error){if(error instanceof Error&&error.message==="Kontrolu duplicit se nepodařilo provést. Zkuste to prosím znovu.")throw error;console.error(JSON.stringify({event:"clients.duplicate_check.transport_failed",endpoint,status:"transport_error",correlationId:"viz frontend.mutation.transport_error",errorName:error instanceof Error?error.name:"Error"}));throw new Error("Kontrolu duplicit se nepodařilo provést. Zkuste to prosím znovu.");}}
  ...
  async changeBuyer(input:{salesCaseId:string;buyers:Array<{partyId:string;role:"buyer"|"co_buyer";isPrimary:boolean;share?:number|null}>;reason:string;idempotencyKey:string}){const response=await apiFetch(`/api/sales/cases/${input.salesCaseId}/buyer`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(input)});if(response.ok)return response.json() as Promise<{id:string}>;const payload=await response.json().catch(()=>({})) as {error?:string};throw new Error(payload.error||"Postoupení smlouvy se nepodařilo dokončit");}
}
