...
class ApiAdminRepository implements AdminRepository {
  ...
  async setRolePermissions(roleId:string,permissionCodes:string[]){const response=await fetch(`/api/identity/admin/roles/${roleId}`,{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({permissionCodes})});if(response.ok)return;if(response.status!==503)throw new Error((await response.json().catch(()=>({}))).error??"Oprávnění role nelze upravit");const snapshot=readPreview();const role=snapshot.roles.find(item=>item.id===roleId);if(!role)throw new Error("Role nebyla nalezena");validateSystemRole(role.code,permissionCodes);snapshot.roles=snapshot.roles.map(item=>item.id===roleId?{...item,permissionCodes,history:[{occurredAt:new Date().toISOString(),actor:"Iva Novotná"},...(item.history??[]).slice(0,4)]}:item);writePreview(snapshot);}
}
