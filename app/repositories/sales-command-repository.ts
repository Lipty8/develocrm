...
async function mutate(url:string,method:string,body:unknown){const response=await fetch(url,{method,headers:{"content-type":"application/json"},body:JSON.stringify(body)});if(response.ok)return false;if(response.status===503)return true;const payload=await response.json().catch(()=>({})) as {error?:string};throw new Error(payload.error||"Obchodní operaci se nepodařilo dokončit");}
function store(key:string,value:unknown){if(typeof window==="undefined")return;const rows=JSON.parse(localStorage.getItem("develocrm.sales.commands")||"{}");rows[key]??=[];rows[key].unshift({...value as object,recordedAt:new Date().toISOString()});localStorage.setItem("develocrm.sales.commands",JSON.stringify(rows));}
