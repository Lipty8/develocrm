...
export type ClientChangeRecord={id:string;projectId:string;projectName:string;unitId:string;unitCode:string;partyId:string;partyName:string;salesCaseId:string|null;title:string;description:string|null;sourceType:"individual"|"catalog";catalogItemCode:string|null;category:string;status:string;surchargeAmount:number|null;currency:string;requestedAt:string;dueAt:string|null;createdAt:string;updatedAt:string;history:Array<{id:string;fromStatus:string;toStatus:string;note:string|null;actor:string;occurredAt:string}>};
...
export const clientChangeRepository={
 ...
 async transition(id:string,status:string,note:string){const response=await apiFetch(`/api/client-changes/${encodeURIComponent(id)}/status`,{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({status,note})});return payload(response) as Promise<ClientChangeRecord>;},
};
