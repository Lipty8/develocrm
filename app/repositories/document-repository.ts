...
export type DocumentRecord = {
  ...
  clientChanges:string[];complaints:string[];handovers:string[];
...
export type DocumentFilters={query?:string;category?:string;typeCode?:string;status?:string;projectId?:string;unitId?:string;partyId?:string;contractId?:string;clientChangeId?:string;complaintId?:string;handoverId?:string};
...
export type DocumentWorkflowLink={type:"client_change"|"complaint"|"handover";targetId:string};
...
export interface DocumentRepository {
  ...
  listClientChange(clientChangeId:string,signal?:AbortSignal):Promise<DocumentListResponse>;
  listComplaint(complaintId:string,signal?:AbortSignal):Promise<DocumentListResponse>;
  listHandover(handoverId:string,signal?:AbortSignal):Promise<DocumentListResponse>;
  ...
  link(documentId:string,target:DocumentWorkflowLink):Promise<void>;
...
class ApiDocumentRepository implements DocumentRepository {
  ...
  async listClientChange(clientChangeId:string,signal?:AbortSignal){return mergePreview(await requestDocuments({clientChangeId},signal),{clientChangeId});}
  async listComplaint(complaintId:string,signal?:AbortSignal){return mergePreview(await requestDocuments({complaintId},signal),{complaintId});}
  async listHandover(handoverId:string,signal?:AbortSignal){return mergePreview(await requestDocuments({handoverId},signal),{handoverId});}
  ...
  async link(documentId:string,target:DocumentWorkflowLink){
    const response=await apiFetch(`/api/documents?documentId=${encodeURIComponent(documentId)}&action=link&targetId=${encodeURIComponent(target.targetId)}`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({linkType:target.type})});
    if(!response.ok)throw await responseError(response,"Dokument se nepodařilo připojit");
  }
...
function matches(document:DocumentRecord,filters:DocumentFilters){const q=(filters.query??"").toLowerCase();return(!q||`${document.name} ${document.typeName} ${document.projectName} ${document.units.join(" ")} ${document.parties.join(" ")}`.toLowerCase().includes(q))&&(!filters.projectId||document.projectId===filters.projectId)&&(!filters.unitId||document.units.includes(filters.unitId))&&(!filters.partyId||document.parties.includes(filters.partyId))&&(!filters.contractId||document.contracts.includes(filters.contractId))&&(!filters.clientChangeId||document.clientChanges.includes(filters.clientChangeId))&&(!filters.complaintId||document.complaints.includes(filters.complaintId))&&(!filters.handoverId||document.handovers.includes(filters.handoverId))&&(!filters.category||document.category===filters.category)&&(!filters.typeCode||document.typeCode===filters.typeCode)&&(!filters.status||document.status===filters.status);}
...
export function mediaDocument(media:MediaLink):DocumentRecord{return{
  ...
  storageProvider:"preview_media",webUrl:media.url,etag:null,sensitivity:"normal",createdAt:"",updatedAt:"",author:"Preview media",version:"aktuální",units:[media.entityId],parties:[],contracts:[],salesCases:[],clientChanges:[],complaints:[],handovers:[],versions:[],events:[],
};}
