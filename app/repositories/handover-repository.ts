...
export type HandoverRecord={id:string;projectId:string;project:string;unitId:string;unit:string;scheduledAt:string;client:string;owner:string;salesCaseId:string|null;status:string;readiness:number;attention:string|null;place:string|null;note:string|null;completedAt:string|null;participants:Array<{partyId:string;name:string;role:string}>};
...
function previewHandovers():HandoverRecord[]{
  ...
  return candidates.map((unit,index)=>{const date=addCalendarDays(new Date(),offsets[index]);date.setHours(times[index][0],times[index][1],0,0);return{id:`preview-handover-${unit.id}`,projectId:unit.project,project:unit.project,unitId:unit.backendId??unit.id,unit:unit.id,scheduledAt:date.toISOString(),client:unit.client??"Bez klienta",owner:index%2?"Martin Jelínek":"Iva Novotná",salesCaseId:null,status:index===0?"ready":index===1?"in_progress":"planned",readiness:index===0?100:Math.max(65,96-index*4),attention:unit.attention??(index===2?"Chybí potvrzení klienta":null),place:null,note:null,completedAt:null,participants:[]};});
}
