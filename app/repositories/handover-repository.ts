...
export interface HandoverRepository{
  list(input:{projectId?:string;status?:string;owner?:string;query?:string;sort?:string;direction?:"asc"|"desc"},signal?:AbortSignal):Promise<HandoverRecord[]>;
...
class ApiHandoverRepository implements HandoverRepository{
  async list(input:{projectId?:string;status?:string;owner?:string;query?:string;sort?:string;direction?:"asc"|"desc"},signal?:AbortSignal){
    const query=new URLSearchParams();if(input.projectId)query.set("projectId",input.projectId);if(input.status)query.set("status",input.status);if(input.owner)query.set("ownerId",input.owner);if(input.query)query.set("query",input.query);if(input.sort)query.set("sort",input.sort);if(input.direction)query.set("direction",input.direction);
