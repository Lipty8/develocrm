...
export async function GET(request:Request){
  ...
  if(!backendUrl||!tenantId||!authorization)return Response.json({currentPrices:Object.fromEntries(units.map(unit=>[unit.id,unit.price])),priceHistories:unitPriceHistories,contracts:contracts.map(contract=>({...contract,id:contract.id??`preview-contract-${slug(contract.project)}-${slug(contract.unit)}-${slug(contract.type)}`,statusCode:contract.statusCode??statusCode(contract.state)})),contractSummary:{},source:"preview-seed"} satisfies CommercialSnapshot);
