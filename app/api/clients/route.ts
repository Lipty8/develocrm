...
export async function GET(request:Request){
  ...
  const scopedClients=incoming.get("projectId")?clients.filter(client=>client.projectIds?.includes(incoming.get("projectId")!)||client.unitRelations?.some(relation=>relation.projectId===incoming.get("projectId"))):clients;
  if(!backendUrl||!tenantId||!authorization)return serverDataMode()==="browser"
    ?incoming.has("page")?browserFallbackResponse({clients:scopedClients.slice((Math.max(1,Number(incoming.get("page"))||1)-1)*(Number(incoming.get("pageSize"))||25),Math.max(1,Number(incoming.get("page"))||1)*(Number(incoming.get("pageSize"))||25)),total:scopedClients.length,page:Math.max(1,Number(incoming.get("page"))||1),pageSize:Number(incoming.get("pageSize"))||25,source:"preview-seed"}):browserFallbackResponse({clients:scopedClients,unitContexts:unitCommercialContexts,source:"preview-seed"} satisfies ClientSnapshot)
